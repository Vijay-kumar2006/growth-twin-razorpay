...
exports.scoreAddons = scoreAddons;
exports.simulateConstraintTradeoffs = simulateConstraintTradeoffs;
exports.generatePaymentRecoveryOptions = generatePaymentRecoveryOptions;
function scoreAddons(baseItems, availableAddons, intent) {
    ...
    return availableAddons.map(addon => {
        ...
        const isCompatible = baseItems.some(base => base.tags.some(tag => addon.tags.includes(tag)) || addon.tags.includes('universal'));
        if (isCompatible) {
            score += 10;
 ...
 */
function simulateConstraintTradeoffs(baseItems, availableAddons, intent, maxUnapprovedThreshold = 20000) {
    ...
    const matchedAddons = availableAddons.filter(a => a.tags.some(t => intent.requestedTags.includes(t)) || hardConstraints.some(h => a.tags.includes(h)));
 ...
 */
function generatePaymentRecoveryOptions(failedQuote, failureReason = 'GATEWAY_CARD_NETWORK_TIMEOUT', maxUnapprovedThreshold = 20000) {
...
//# sourceMappingURL=revenue-bundle.js.map